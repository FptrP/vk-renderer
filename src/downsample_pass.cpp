...
DownsamplePass::DownsamplePass() {
  ...
  downsample_gbuffer.set_rendersubpass({true, {VK_FORMAT_R16G16_SFLOAT, VK_FORMAT_D24_UNORM_S8_UINT}});
  ...
  rendergraph::ImageResourceId out_normal)
{
  ...
  if (depth_ext.width != norm_ext.width || depth_ext.height != norm_ext.height) {
    throw std::runtime_error {"Depth and normal texture have different sizes"};
  }

  downsample_gbuffer.set_rendersubpass({true, {norm_desc.format, depth_desc.format}});
...
{
  run_downsample_gbuff(graph, src_normals, depth, out_normals);
